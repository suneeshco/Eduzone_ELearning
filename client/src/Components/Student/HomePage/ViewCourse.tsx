...
const ViewCourse: React.FC = () => {
  ...
  return (
    ...
    <div>
      <div className='flex space-between pt-20 pb-20 mt-20 bg-gray-600 '>
        <div className="w-full sm:w-3/5 m-4 sm:m-20">
          <h1 className="text-4xl text-white font-bold mb-4 " style={{ fontFamily: 'Arial, sans-serif' }}>{courseDetails?.courseName}</h1>
          <h1 className="text-xl text-white pt-3 mb-4">{courseDetails?.courseDescription}</h1>

          <div className="flex items-center mb-1 space-x-1 rtl:space-x-reverse">
            {[...Array(5)].map((_, index) => (
              <svg
                key={index}
                className={`w-4 h-4 ${index < (courseDetails?.rating || 0) ? 'text-yellow-300' : 'text-gray-300 dark:text-gray-500'
                  }`}
                aria-hidden="true"
          ...
          {!courseDetails?.students.includes(userInfo?._id) && (
            <div className='pt-5  '>
              <h1 className="text-2xl text-white font-bold mb-4 ">Price : ₹ {courseDetails?.courseFee}</h1>
            </div>)}

        </div>
        <div>
          <div className='bg-slate-50 mx-8 border-b border-black shadow-2xl m-10 '>
            <div>
      ...
      <div className='bg-white sm:m-20 flex flex-col'>
  <div className="bg-white rounded-lg p-6 mt-16">
    <h2 className="text-2xl font-bold mb-4">Topics That You Are Going To Learn</h2>
    {lessonDetails.map((lesson, index) => (
      <div className="md:w-1/2 flex items-center justify-between bg-violet-300 rounded p-2 mb-2" key={lesson._id}>
        <h2 className="flex-grow p-2">{index + 1 + ") " + lesson.lessonTitle}</h2>
        <div className="flex items-center space-x-2">
          {courseDetails?.students.includes(userInfo?._id) && (
            <button
              onClick={() => handleLessonClick(lesson.lessonVideo)}
  ...
  Close
      ...
      <ChatPageStudent instructorId={courseDetails.instructorId}/>
         ...
         <img src={chatIcon} className='rounded-full fixed bottom-10 w-24 h-24 ring-2 ring-indigo-500  cursor-pointer' alt="chat" onClick={()=>{setChatWindow(true)}} />
      ...
      {userInfo && courseDetails?.students.includes(userInfo?._id) && (
        <div className='border-t  sm:m-20 p-6 '>
          <h2 className="text-3xl font-bold mb-4 text-center  ">Rate Your Course</h2>
          <StarRating initialRating={rating} ratings={rating} onChange={handleRatingChange} />
          <div className='flex justify-center items-center'>
            <textarea
              id="review"
              name="review"
              className="mt-5  p-4 block w-1/2 border border-black  rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
          ...
          <div className="flex justify-center items-center pt-5">
            <button
              ...
              className=" w-1/5 bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white font-semibold py-3 px-6 rounded-full shadow-lg transform hover:scale-105 transition duration-300 ease-in-out"
            >
      ...
      <div className='border-t  sm:m-20 p-6'>
        <h2 className="text-3xl font-bold mb-4   ">Reviews And Ratings</h2>

        {ratingDetails.map((ratings) => (
          <article className='w-1/2 bg-slate-50 p-5 border'>
            <div className="flex items-center mb-4">
              <img className="w-10 h-10 me-4 rounded-full" src={ratings.studentId.photo} alt="" />
              <div className="font-medium dark:text-white">
                <p>{ratings.studentId.firstname} <time dateTime="2014-08-16 19:00" className="block text-sm text-gray-500 dark:text-gray-400">Rated On {new Date(ratings.createdAt).toLocaleDateString()}</time></p>
              </div>
            </div>
            <div className="flex items-center mb-1 space-x-1 rtl:space-x-reverse">
              {[...Array(5)].map((_, index) => (
                <svg
                  key={index}
                  className={`w-4 h-4 ${index < ratings.rating ? 'text-yellow-300' : 'text-gray-300 dark:text-gray-500'
                    }`}
                  aria-hidden="true"
                  xmlns="http://www.w3.org/2000/svg"
                  fill="currentColor"
            ...
            <p className="mb-2 text-gray-500 dark:text-gray-400">{ratings.review}</p>
